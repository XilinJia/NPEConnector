...
open class MediaSearcher(val name_: String, val serviceId: Int) : IMediaSearchProvider.Stub() {
    val service by lazy { try { NewPipe.getService(serviceId) } catch (e: ExtractionException) { throw ExtractionException("NewPipe service not found") } }
    var page: Page? = null
    ...
    override fun search(query: String, limit: Int): List<EpisodeIPC> {
        try {
            ...
            while (items.isNotEmpty()) {
                for (m in items) if (m is StreamInfoItem) podResults.add(m.toEpisodeIPC())
    ...
    open fun contentFilter(): List<String> = listOf("videos")
